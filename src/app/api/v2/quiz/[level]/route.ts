...
interface ParsedContent extends Partial<OrderingQuestion & ChoiceQuestion> {
...
export async function GET(request: NextRequest, { params }: Props) {
  ...
  try {
    ...
    async function generateOne(word: WordRecord, index: number): Promise<GeneratedQuestion> {
      ...
      const parsedContent = JSON.parse(content) as ParsedContent;
      const questionText = parsedContent.question_text ?? '';

      if (questionType === 'construction') {
        return {
          word_id: word.id,
          type: (parsedContent.type as OrderingQuestion['type']) ?? 'ordering',
      ...
      return {
        word_id: word.id,
        type: questionType as ChoiceQuestion['type'],
        question: questionText,
        options: parsedContent.options ?? [],
        correct_answer: parsedContent.correct_answer ?? undefined,
        pinyin: parsedContent.pinyin ?? undefined,
        sentence:
          parsedContent.sentence ?? parsedContent.sentence_raw ?? undefined,
        marked_sentence: parsedContent.marked_sentence ?? undefined,
        situation: parsedContent.situation ?? undefined,
        word_display: parsedContent.word_display ?? undefined,
      };
    }

    const results = await Promise.allSettled(
      allWords.map((word, idx) => generateOne(word, idx))
    );
